...
Este trabajo documenta la implementación de sistemas de control con dificultad progresiva:

1.  Implementar secuencias de control para salidas digitales usando temporización no bloqueante y **aritmética de punteros**.
2.  Diseñar una **Máquina de Estados Finitos (FSM)** para la gestión de un sistema de semáforos.
3.  Desarrollar una **Interfaz Gráfica (GUI)** en HMI para el control de salidas **PWM**.
4.  Implementar un controlador **PID discreto** para la regulación de velocidad (RPM) de un motor DC, incluyendo adquisición de datos por **interrupciones**.
...
* Desarrollo de una interfaz con **SpinBoxes** en el HMI para ajustar el **Duty Cycle (0-100%)** de dos LEDs.
...
* **Modelado del Sistema:** Obtención de la Función de Transferencia del Motor DC mediante identificación de sistemas: $$G(s)=\frac{4.444}{0.0054s+1}$$.
...
* **Verificación:** El controlador, sintonizado a partir del modelo, demostró **estabilidad**, **bajo sobrepaso** y un error en estado estacionario **inferior al 5%**, cumpliendo con los requisitos de rendimiento y robustez ante perturbaciones.
